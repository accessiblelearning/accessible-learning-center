(() => {
  ...
  function render() {
    ...
    if (!query) {
      status.textContent = "Enter one or more words to search manuals, lessons, final quizzes, and practice resources.";
      return;
    }
  ...
  fetch("search-index.json?v=resource-search-1", { cache: "no-store" })
    ...
    .then(data => {
      ...
      if (queryInput.value.trim()) render();
      else status.textContent = "Search is ready. Enter one or more words to search manuals, lessons, final quizzes, and practice resources.";
    })
    .catch(() => {
