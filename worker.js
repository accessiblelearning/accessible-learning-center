  ...
  async fetch(request, env) {
    ...
    if (url.pathname === "/health") {
      return Response.json(
        { status: "ok", message: "Accessible Learning API is running." },
        { headers: cors }
      );
    }

    // Create or recognize a Student ID
    if (url.pathname === "/student" && request.method === "POST") {
      ...
      return Response.json(
        { success: true, student_id: studentId },
        { headers: cors }
      );
    }
    ...
    if (url.pathname === "/progress" && request.method === "POST") {
      ...
      await env.DB.prepare(`
        ...
        (student_id, course, lesson_number, status, attempts, updated_at)
      ...
      `)
        .bind(studentId, course, lessonNumber, status)
        .run();
    ...
    if (url.pathname === "/progress" && request.method === "GET") {
      ...
      const results = await env.DB.prepare(`
        SELECT course, lesson_number, status, attempts,
               last_score, feedback, updated_at
        FROM progress
        WHERE student_id = ?
      ...
      return Response.json(results.results, { headers: cors });
    }
